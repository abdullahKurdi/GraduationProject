<?php
    ...
    foreach ($category as $a){
        ...
        if ($a['CatID']==$catid){
            ...
            // echo 'where Parent = ' . $a['CatID'];
            if ($a['Parent'] != 0){
                $sUB =  getCat('where CatID = ' . $a['Parent']);
                foreach ($sUB as $su){
                    $visible = $su['Visibility'];
                    if ($visible == 0){
                        $visible = $a['Visibility'];
                    }
                }
        ...
        // echo $Parent;
     if($catid !== 0 && in_array($catid,$categories) && $visible == 0){
        $limit =6;
        $page = isset($_GET['page']) && is_numeric($_GET['page']) ? intval($_GET['page']): 1 ;
        $start = ($page - 1) * $limit;
        $items = getItem($Parent,'Cat_ID',$catid , $_SESSION['TypeOfAuction'], $start , $limit);
        $countITEM = getItemCount($Parent,'Cat_ID',$catid, $_SESSION['TypeOfAuction']);
        $total = $countITEM['id'];
        $pages = ceil($total/$limit);
        $Previous = $page - 1 ;
        $Next = $page + 1;
        echo '<div class="row aabbcc alert alert-dark"> '.lang($catname).'  '.' <spna class="aa"> '.lang('Page').' </span> </div>';
        // echo getcwd()."\n"."</br>";
                ...
                if(!empty($result)){
                 ...
                 echo '<div class="navigation row">
                                ...
                                <li class="page-item ';if($page==1){echo 'disabled';}echo' ">
                                    <a class="page-link" href="Category.php?pageid='.$catid .'&page='.$Previous.'" tabindex="-1">'.lang('Previous').'</a>
                                </li>
                                ';
                                for($i = 1 ; $i <= $pages ; $i++){
                                echo '<li class="page-item"><a class="page-link" href="Category.php?Catid='.$catid .'&page='.$i.'">'.$i.'</a></li>';
                                    }
                                echo '<li class="page-item ';if($page==$pages){echo 'disabled';}echo'">
                                    <a class="page-link" href="Category.php?Catid='.$catid .'&page='.$Next.'">'.lang('Next').'</a>
                                </li>
                                </ul>
                            </nav>
                    </div>';
            }else{
                echo '<div class="ABCDEF alert alert-info"><strong>'.lang('There Is No Items').'</strong></div>';
            }
        }else if($catid == 0){
            header('Location:index.php'); // redirect to dashboard page
            exit();
        }else{
            echo '<div class="ABCDEF alert alert-info"><strong>'.lang('there is no category in this id').'</strong></div>';
        }
            echo "</div>";
